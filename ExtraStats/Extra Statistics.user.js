...
// ==/UserScript==
(function() {
    // GITHUB GISTS API FUNCTIONS ////////////////////////////////////////////////
    
    // GitHub Gists API configuration
    var GITHUB_GISTS_API = {
        baseUrl: 'https://api.github.com/gists',
        token: null, // Will be set by user
        isEnabled: false
    };
    
    // Initialize GitHub Gists settings from localStorage
    function initGithubSettings() {
        var settings = localStorage.getItem('wod_github_gists_settings');
        if (settings) {
            try {
                var parsed = JSON.parse(settings);
                GITHUB_GISTS_API.token = parsed.token;
                GITHUB_GISTS_API.isEnabled = parsed.enabled || false;
            } catch (e) {
                console.log('Failed to parse GitHub Gists settings:', e);
            }
        }
    }
    
    // Save GitHub Gists settings to localStorage
    function saveGithubSettings() {
        var settings = {
            token: GITHUB_GISTS_API.token,
            enabled: GITHUB_GISTS_API.isEnabled
        };
        localStorage.setItem('wod_github_gists_settings', JSON.stringify(settings));
    }
    
    // Upload content to GitHub Gists
    function uploadToGithubGists(content, filename, description) {
        return new Promise(function(resolve, reject) {
            if (!GITHUB_GISTS_API.isEnabled || !GITHUB_GISTS_API.token) {
                reject(new Error('GitHub Gists not configured'));
                return;
            }
            
            var gistData = {
                description: description || 'WoD Battle Report',
                public: false,
                files: {}
            };
            ...
            xhr.open('POST', GITHUB_GISTS_API.baseUrl, true);
            xhr.setRequestHeader('Authorization', 'token ' + GITHUB_GISTS_API.token);
                ...
                if (xhr.readyState === 4) {
                    if (xhr.status === 201) {
                        ...
                        resolve({
                            ...
                            id: response.id,
                            rawUrl: response.files[filename].raw_url
                        });
                    } else {
                        reject(new Error('Failed to upload to GitHub Gists: ' + xhr.status + ' ' + xhr.statusText));
                    }
                }
            };
    ...
    // Show GitHub Gists settings dialog
    function showGithubSettingsDialog() {
        var dialog = document.createElement('div');
        dialog.style.cssText = 'position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: white; border: 2px solid #333; padding: 20px; z-index: 10000; box-shadow: 0 4px 8px rgba(0,0,0,0.3);';
        
        dialog.innerHTML = `
            <h3>GitHub Gists 设置</h3>
            <p>配置GitHub Gists以自动上传战报文件</p>
            <label>
                <input type="checkbox" id="github_enabled" ${GITHUB_GISTS_API.isEnabled ? 'checked' : ''}>
                启用GitHub Gists上传
            </label><br><br>
            <label>
                GitHub Personal Access Token:<br>
                <input type="password" id="github_token" value="${GITHUB_GISTS_API.token || ''}" style="width: 300px;">
            </label><br><br>
            <p style="font-size: 12px; color: #666;">
                获取Token: <a href="https://github.com/settings/tokens" target="_blank">https://github.com/settings/tokens</a><br>
                需要权限: gist (创建gists)
            </p>
            <br>
            <button onclick="saveGithubSettingsAndClose()">保存</button>
            <button onclick="closeGithubSettings()">取消</button>
        `;
        
        document.body.appendChild(dialog);
        
        window.saveGithubSettingsAndClose = function() {
            GITHUB_GISTS_API.isEnabled = document.getElementById('github_enabled').checked;
            GITHUB_GISTS_API.token = document.getElementById('github_token').value.trim();
            saveGithubSettings();
            document.body.removeChild(dialog);
            alert('GitHub Gists设置已保存');
        };
        
        window.closeGithubSettings = function() {
            document.body.removeChild(dialog);
        };
    }
    ...
    function OnCountStat() {
        try {
            ...
            progress = setInterval(Stat.ShowProgress.bind(Stat), 100);
	...
	function ReadDuelPage() {
        ...
        gZip.generateAsync({
            ...
            var filename = "wodlog" + '_' + Math.random().toString(36).substr(2, 9) + ".zip";
            
            if (GITHUB_GISTS_API.isEnabled && GITHUB_GISTS_API.token) {
                // Upload to GitHub Gists
                var reader = new FileReader();
                reader.onload = function() {
                    var base64Content = reader.result.split(',')[1];
                    uploadToGithubGists(base64Content, filename, gTitle + " - WoD Battle Report")
                        .then(function(result) {
                            alert('战报已上传到GitHub Gists!\n链接: ' + result.url);
                            infodiv.innerHTML = "";
                            gResponseDiv.innerHTML = "";
                            gIsWorking = false;
                        })
                        .catch(function(error) {
                            alert('上传到GitHub Gists失败: ' + error.message + '\n将下载到本地...');
